   ...
   myfunc(val)
    {
        for(let i=0;i<val.length;i=i+2)
        {
            ...
            <div className="col-md-6 padding-0">
              <div className="stylhovediv" style={{backgroundImage:"url("+val[i].image+")"}}>
              <div className="insidedivhover" id="product1" src={val[i].image} >
              <button className="probutt" onClick={(e)=>this.adddata(e,val[i].description,i,val[i].name)}>{val[i].name}</button> 
              </div>
              </div>
            </div>

            <div className="col-md-6 padding-0">
              <div className="stylhovediv" style={{backgroundImage:"url("+val[i+1].image+")"}}>
              <div className="insidedivhover" id="product1" src={val[i+1].image} >
              <button className="probutt" onClick={(e)=>this.adddata(e,val[i+1].description,i,val[i+1].name)}>{val[i+1].name}</button> 
              </div>
              </div>
            </div>
            </div>
            <div className="proddiv" id={i} style={{display:"none",fontSize: "15px",padding:20 , margin:0}}></div>
            </div>);            
            }
            else
            {
              ...
              <button className="probutt" onClick={(e)=>this.adddata(e,val[i].description,i,val[i].name)}>{val[i].name}</button> 
                ...
                <div className="proddiv" id={i} style={{display:"none", fontSize: "15px",padding:20 }}></div>
                </div>);                
            }
        }
...
/*
      <div className="row">
            ...
            {this.props.location.state.products.map((i)=>{
                ...
                <button className="probutt" onClick={(e)=>this.adddata(e,i.description)}>{i.name}</button> 
